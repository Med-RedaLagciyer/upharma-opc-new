$(document).ready(function () {
        ...
        if (e.key === 'Enter') {
            ...
            try {
                const request = await axios.post(
                    Routing.generate('app_pharmacy_livraison_livre_details', { codeLivraison: inputValue })
                );
    ...
    if ($.fn.dataTable.isDataTable("#list_Livraison_livre")) {
        $("#list_Livraison_livre").DataTable().clear().destroy();
    }

    var livraison_array = [];

    var table = $("#list_Livraison_livre").DataTable({
        lengthMenu: [
        ...
        order: [[0, "asc"]],
        ajax: {
            url: Routing.generate("app_pharmacy_livraison_livre_list"),
            type: "get",
            data: function (d) {
        ...
        columnDefs: [
            {
                ...
                render: function (data, type, full, meta) {
                    ...
                    if(full.isValide){
                        return `<input disabled type="checkbox" class="" data-id="${data}">`;
                    }else{
        ...
        initComplete: function () {
            ...
            $('#list_Livraison_livre tbody').on('change', 'input.selectLivraison', function () {
                var id = $(this).data('id');
    ...
    function updateSelectAllCheckboxState() {
        var allChecked = $("#list_Livraison_livre tbody input.selectLivraison").length === $("#list_Livraison_livre tbody input.selectLivraison:checked").length;
    ...
    $('body').on('click', '.detailsLiv', async function (e) {
        ...
        try {
            const request = await axios.post(
                Routing.generate('app_pharmacy_livraison_livre_details', { idLivraison: id_livraison })
            );
            const response = await request.data;
            $("#detailsModal #detailsBody").html("");
            $("#detailsModal #detailsBody").html(response["detailLivraison"]);
    ...
    $('body').on('click', '#validerMasse', async function (e) {
        e.preventDefault();
    ...
    $('body').on('click', '#saveValidation', async function (e) {
        e.preventDefault();
